...
const PaymentsMenu = (props) => {
    ...
    return (
        <div className="payments-menu">
            <PaymentModal show={modal === "payment"} hideModal={hideModal} custID={custID} payment={selectedPayment} />
                ...
                {payments && payments.map((payment) => (
                    <div className="header flex flex-row justify-between bg-white my-4 py-4 px-8 rounded-md drop-shadow-md" key={payment.paymentid}>
                        ...
                        <div className="options flex flex-row mt-2">
                            <button className="mx-2 h-0" onClick={(e) => {handleEdit(e, payment)}}><FaEdit className="fill-emerald-600"/></button>
                            <button className="mx-2 h-0" onClick={(e) => {handleDelete(e, payment.paymentid)}}><FaTrashAlt className="fill-emerald-600"/></button>
                        </div>
                    </div>
                ))}  
            </div>
